		...
		public async Task ConfigureAsync(
	            ...
	            int? port = 0)
	        {
	            ...
	            if (isServer)
	            {
	                ...
	                await server.UpServerAsync(host, port, new CancellationToken());
	            }
	            else
	            {
	                if (port.HasValue) // Если port не равен null
	                {
	                    ...
	                    await client.ConnectToServerAsync(host, port.Value); // Доступ к значению port
	                }
	                else
	                {
	                    ...
	                    IClient client = factory.CreateClient();
	                    await client.ConnectToServerAsync(host, 5000); // Пример с дефолтным портом
	                }
	            }
	        }
	    }
