...
public class TcpClientHandler : ITcpClientHandler
{
			...
			if (_client.Connected)
			{
				...
				do
				{
					try
					{
						...
						_logger.LogWarning("Сообщение получено.");
					}
					catch (Exception ex)
					{
	...
	private void BindLocalEndpoint()
	{
		...
		{
			// Если порт указан, привязываемся к нему
		...
		{
			// Если порт не указан (== 0), система сама выберет свободный порт
			var localEndPoint = new System.Net.IPEndPoint(System.Net.IPAddress.Parse(_clientHost ?? "0.0.0.0"), 0);
			_client.Client.Bind(localEndPoint);

			// Получаем фактический порт, который выбрала ОС
			var assignedPort = ((System.Net.IPEndPoint)_client.Client.LocalEndPoint).Port;
			_clientPort = assignedPort;
			_logger.LogInformation($"Клиент привязан к {localEndPoint}, выбран порт: {assignedPort}");
		}
	}
	...
	public async Task ReceiveMessagesAsync()
	{
		...
		do
		{
			try
			{
				...
				_logger.LogInformation($"Получено сообщение: {receivedMessage}");
			}
			catch (Exception ex)
			{
				_logger.LogError(ex, "Ошибка при чтении данных.");
