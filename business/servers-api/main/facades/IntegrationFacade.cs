...
namespace servers_api.main.facades
{
	public class IntegrationFacade(
		IQueueFacade queueFacade,
		IProcessingFacade processingFacade) : IIntegrationFacade
	{
		...
		public Task<ResponseIntegration> TeachBpmAsync(
			CombinedModel model,
			CancellationToken stoppingToken)
			=> processingFacade.ExecuteTeachAsync(model, stoppingToken);

		public Task<ResponseIntegration> ConfigureNodeAsync(
			CombinedModel model,
			CancellationToken stoppingToken)
